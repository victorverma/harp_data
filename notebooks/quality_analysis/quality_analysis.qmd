...
max_abs_lon_range <- harp_tbl %>%
  filter(max_abs_lon <= 90) %>%
  pull(max_abs_lon) %>%
  range()
breaks <- seq(
...
prop_tbl <- harp_tbl %>%
  ...
  summarize(
    ...
    low_quality_prop = mean(is_quality_low, na.rm = TRUE),
    num_recs = n(),
    se_upper_bound = 1 / (2 * sqrt(num_recs))
  ) %>%
  ungroup()
```

Below are the counts of records and upper bounds on the standard errors for the proportions for the various bins.
```{r}
prop_tbl %>%
  select(max_abs_lon, num_recs, se_upper_bound) %>%
  kable(
    "html",
    digits = 3,
    # col.names = c("Maximum Absolute Longitude Bin", "Number of Records", "SE Upper Bound"),
    # align = "crr",
    format.args = list(big.mark = ",")
  ) %>%
  kable_styling()
...
prop_tbl %>%
  pivot_longer(
    cols = !c(max_abs_lon, midpoint, num_recs, se_upper_bound),
    names_to = "prop_type", names_pattern = "([a-z_]+)_prop",
    values_to = "prop_val"
  ) %>%
  ggplot(aes(midpoint, prop_val, linetype = prop_type)) +
...
lon_min_range <- harp_tbl %>%
  filter(between(LON_MIN, -90, 90)) %>%
  pull(LON_MIN) %>%
  range()
breaks <- seq(
  floor(lon_min_range[1]), ceiling(lon_min_range[2]), length.out = 20
)
midpoints <- (breaks[-length(breaks)] + breaks[-1]) / 2
breaks[length(breaks)] <- Inf
prop_tbl <- harp_tbl %>%
  filter(between(LON_MIN, -90, 90)) %>%
  mutate(
    LON_MIN = cut(LON_MIN, breaks = breaks, right = FALSE),
    midpoint = midpoints[as.integer(LON_MIN)]
  ) %>%
  group_by(LON_MIN, midpoint) %>%
  summarize(
    missing_rec_prop = mean(is_rec_missing),
    incomplete_rec_prop = mean(is_rec_incomplete),
    low_quality_prop = mean(is_quality_low, na.rm = TRUE),
...
```{r}
prop_tbl %>%
  select(LON_MIN, num_recs, se_upper_bound) %>%
  kable(
    "html",
    digits = 3,
    # col.names = c("Minimum Longitude Bin", "Number of Records", "SE Upper Bound"),
    # align = "crr",
    format.args = list(big.mark = ",")
  ) %>%
  kable_styling()
...
prop_tbl %>%
  pivot_longer(
    cols = !c(LON_MIN, midpoint, num_recs, se_upper_bound),
    names_to = "prop_type", names_pattern = "([a-z_]+)_prop",
    values_to = "prop_val"
  ) %>%
  ggplot(aes(midpoint, prop_val, linetype = prop_type)) +
  ...
  labs(
    x = expression("Minimum Longitude (" * degree * ")"),
    y = "Proportion Value",
    linetype = "Proportion Type"
  ) +
  theme_bw() +
...
lon_max_range <- harp_tbl %>%
  filter(between(LON_MAX, -90, 90)) %>%
  pull(LON_MAX) %>%
  range()
breaks <- seq(
  floor(lon_max_range[1]), ceiling(lon_max_range[2]), length.out = 20
)
midpoints <- (breaks[-length(breaks)] + breaks[-1]) / 2
breaks[length(breaks)] <- Inf
prop_tbl <- harp_tbl %>%
  filter(between(LON_MAX, -90, 90)) %>%
  mutate(
    LON_MAX = cut(LON_MAX, breaks = breaks, right = FALSE),
    midpoint = midpoints[as.integer(LON_MAX)]
  ) %>%
  group_by(LON_MAX, midpoint) %>%
  summarize(
    missing_rec_prop = mean(is_rec_missing),
    incomplete_rec_prop = mean(is_rec_incomplete),
    low_quality_prop = mean(is_quality_low, na.rm = TRUE),
...
```{r}
prop_tbl %>%
  select(LON_MAX, num_recs, se_upper_bound) %>%
  kable(
    "html",
    digits = 3,
    # col.names = c("Maximum Longitude Bin", "Number of Records", "SE Upper Bound"),
    # align = "crr",
    format.args = list(big.mark = ",")
  ) %>%
  kable_styling()
...
prop_tbl %>%
  pivot_longer(
    cols = !c(LON_MAX, midpoint, num_recs, se_upper_bound),
    names_to = "prop_type", names_pattern = "([a-z_]+)_prop",
    values_to = "prop_val"
  ) %>%
  ggplot(aes(midpoint, prop_val, linetype = prop_type)) +
  ...
  labs(
    x = expression("Maximum Longitude (" * degree * ")"),
    y = "Proportion Value",
    linetype = "Proportion Type"
  ) +
  theme_bw() +
...
```{r}
make_sharp_params_plot <- function(one_harp_tbl, harp_num) {
  start_time <- end_time <- NULL
  with(
    one_harp_tbl,
    {
  ...
  one_harp_tbl %>%
    select(T_REC, all_of(sharp_params)) %>%
    pivot_longer(
      cols = !T_REC, names_to = "sharp_param", values_to = "val"
    ) %>%
    ggplot(aes(T_REC, val)) +
    facet_wrap(vars(sharp_param), nrow = 4, ncol = 4, scales = "free_y") +
    geom_line() +
    labs(
      x = "Time", y = "Value",
      title = str_glue("HARP {harp_num}"),
...
one_harp_tbls <- harp_tbl %>%
  nest(.by = HARPNUM, .key = "one_harp_tbl") %>%
  mutate(
    sharp_params_plot = map2(one_harp_tbl, HARPNUM, make_sharp_params_plot)
  )
```
...
pdf(
  here("notebooks/quality_analysis/sharp_params_plots.pdf"),
  width = 12, height = 6
)
with(one_harp_tbls, walk(sharp_params_plot, print))
dev.off()
```

```{r}
impute_sharp_params <- function(one_harp_tbl) {
  tryCatch(
    mutate(one_harp_tbl, across(all_of(sharp_params), na_interpolation)),
...
```{r}
one_harp_tbls <- mutate(
  one_harp_tbls,
  one_harp_tbl = map(one_harp_tbl, impute_sharp_params),
  has_inf = map_lgl(one_harp_tbl, ~ any(.x$has_inf))
)
```

```{r}
bad_harp_nums <- one_harp_tbls %>%
  filter(map_lgl(one_harp_tbl, is.null)) %>%
  pull(HARPNUM)
harp_tbl %>%
  filter(HARPNUM %in% bad_harp_nums) %>%
  select(HARPNUM, all_of(sharp_params)) %>%
  unique()
```

```{r}
harp_tbl2 <- one_harp_tbls %>%
  filter(!map_lgl(one_harp_tbl, is.null), !has_inf) %>%
  select(!c(sharp_params_plot, has_inf)) %>%
  unnest(one_harp_tbl)
```
...
harp_tbl2 %>%
  select(all_of(sharp_params)) %>%
  mutate(is_rec_missing = vec_detect_missing(.)) %>%
  pull(is_rec_missing) %>%
  any()
```

```{r}
harp_tbl2 <- select(harp_tbl2, !(is_rec_incomplete:max_abs_lon))
```

Why $\pm 68^{\circ}$?
```{r}
harp_tbl2 <- filter(harp_tbl2, LON_MIN >= -68, LON_MAX <= 68)
```
...
longest_run <- time_summary_tbl %>%
  ...
  slice_max(days_elapsed)
harp_tbl2 <- filter(
  harp_tbl2, between(T_REC, longest_run$start_time, longest_run$end_time)
)
```
...
```{r}
write_parquet(harp_tbl2, here("data/processed/harp_data2.parquet"))
```
